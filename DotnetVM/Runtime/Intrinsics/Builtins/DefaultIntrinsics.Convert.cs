...
public static partial class DefaultIntrinsics {
    ...
    private static void RegisterConvert(IntrinsicRegistry r) {
        ...
        S("ToInt32", 1, static (ctx, a) => {
            ...
            return s[0].Kind switch {
                ...
                StackKind.Object when s[0].ObjectValue is VmString str =>
                    StackSlot.OfInt32(int.Parse(str.Value, NumberStyles.Integer, ctx.Shared.Culture)),
        ...
        S("ToInt64", 1, static (ctx, a) => {
            ...
            return s[0].Kind switch {
                ...
                StackKind.Object when s[0].ObjectValue is VmString str =>
                    StackSlot.OfInt64(long.Parse(str.Value, NumberStyles.Integer, ctx.Shared.Culture)),
                _ => throw new InvalidOperationException($"Convert.ToInt64 未対応の入力型: {s[0].Kind}"),
            };
        });
        S("ToDouble", 1, static (ctx, a) => {
            var s = new Args(a);
            return s[0].Kind switch {
                ...
                StackKind.Object when s[0].ObjectValue is VmString str => StackSlot.OfFloat(
                    double.Parse(str.Value, NumberStyles.Float | NumberStyles.AllowThousands, ctx.Shared.Culture)),
        ...
        S("ToString", 1, static (ctx, a) => {
            var s = new Args(a);
            return s[0].Kind switch {
                StackKind.Int32 => StackSlot.OfObject(ctx.MakeString(s.Int32(0).ToString(null, ctx.Shared.Culture))),
                StackKind.Int64 => StackSlot.OfObject(ctx.MakeString(s.Int64(0).ToString(null, ctx.Shared.Culture))),
                StackKind.Float => StackSlot.OfObject(ctx.MakeString(s.Float(0).ToString(null, ctx.Shared.Culture))),
        ...
        S("ToByte", 1, static (ctx, a) => ToIntegral(ctx, "ToByte", new Args(a), byte.MinValue, byte.MaxValue));
        S("ToSByte", 1, static (ctx, a) => ToIntegral(ctx, "ToSByte", new Args(a), sbyte.MinValue, sbyte.MaxValue));
        S("ToInt16", 1, static (ctx, a) => ToIntegral(ctx, "ToInt16", new Args(a), short.MinValue, short.MaxValue));
        S("ToUInt16", 1, static (ctx, a) => ToIntegral(ctx, "ToUInt16", new Args(a), ushort.MinValue, ushort.MaxValue));
        S("ToUInt32", 1, static (ctx, a) => ToIntegral(ctx, "ToUInt32", new Args(a), uint.MinValue, uint.MaxValue));
        S("ToUInt64", 1, static (ctx, a) => ToUnsigned64(ctx, "ToUInt64", new Args(a)));
        S("ToSingle", 1, static (ctx, a) => {
            var s = new Args(a);
            var value = s[0].Kind switch {
                ...
                StackKind.Object when s[0].ObjectValue is VmString str =>
                    float.Parse(str.Value, NumberStyles.Float | NumberStyles.AllowThousands, ctx.Shared.Culture),
    ...
    private static StackSlot ToIntegral(IntrinsicContext ctx, string target, Args s, long min, long max) {
        var value = s[0].Kind switch {
            StackKind.Int32 => s.Int32(0),
            StackKind.Int64 => s.Int64(0),
            StackKind.Float => (long)Math.Round(s.Float(0), MidpointRounding.ToEven),
            StackKind.Object when s[0].ObjectValue is VmString str =>
                long.Parse(str.Value, NumberStyles.Integer, ctx.Shared.Culture),
    ...
    private static StackSlot ToUnsigned64(IntrinsicContext ctx, string target, Args s) {
        ...
        var value = s[0].Kind switch {
            ...
            StackKind.Object when s[0].ObjectValue is VmString str =>
                ulong.Parse(str.Value, NumberStyles.Integer, ctx.Shared.Culture),
            _ => throw new InvalidOperationException($"Convert.{target} 未対応の入力型: {s[0].Kind}"),
        };
