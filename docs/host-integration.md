...
JIT は非 EH のスカラー、呼出、`newobj`、配列、フィールド、box/cast、managed ByRef、間接アクセス、値型コピー、
...
VM オブジェクトの確保は `VmHeap.Allocate` に集約されます。一般的なクラス実体、配列、ボックス、
intrinsic 実体ではサイズ判定の型ディスパッチを省略しますが、クォータ検査、アロケーション会計、
GC 追跡への登録は省略しません。したがってホスト側で別の確保経路を作らず、VM の入口を利用してください。
